...
import { boot as aiNewsBoot, tick as aiNewsTick } from './lib/ai-news-processor';
...
serve({ fetch: app.fetch, port, hostname }, (info) => {
  ...
  void (async () => {
    try {
      ...
      logger.warn({ err: (err as Error).message }, 'ai-news boot reset failed');
    }
    await aiNewsTick().catch((err) =>
